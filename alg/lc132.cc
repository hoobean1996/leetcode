...
public:
    ...
    int minCut(string s) {
        ...
        memoPalindrome = vector<vector<optional<bool>>>(s.length(),vector<optional<bool>>(s.length(), nullopt));
    ...
    int findMinCut(string& s, int start, int end, int minCut) {
        ...
        if (start == end || valid(s, start, end)) return 0;
        
        // 如果之前已经有结果了，直接返回
        if (memoCuts[start][end] != -1) return memoCuts[start][end];
    ...
    bool valid(string& s, int start, int end) {
        ...
        if (memoPalindrome[start][end] != nullopt) return (memoPalindrome[start][end] == true);
        
        memoPalindrome[start][end] = (s[start] == s[end] && valid(s, start+1, end-1));
...
// isPalindrome(i, j) = true, if s[i] == s[j] and isPalindrome(i+1, j-1) else false
// O(N^2)
class Solution {
public:
    int minCut(string s) {
        ...
        for(int i=1; i<s.length(); i++) dp[i] = i;
    ...
    void findMinCuts(int start, int end, vector<int>& dp, string s) {
        for(int l=start, r=end; l>=0 && r<s.length() && s[l] == s[r]; l--, r++) {
